...
class TelegramForwarder:
    ...
    async def forward_messages_to_channel(self, source_chat_id, destination_identity):
        ...
        print("Vuoi filtrare per nome utente? (Se si, inseriscilo altrimenti lascia vuoto)")
        ...
        while True:
            ...
            for message in reversed(messages):
                if username and message.sender and message.sender.username:
                    if message.sender.username.lower() != username.lower().strip('@'):
                        continue  # Skip this message if username doesn't match
                ...
                if message.sender.username and str(message.sender.username) == "None":
                    continue
                
                #print("message.sender.username = " + str(message.sender.username))
            ...
            await asyncio.sleep(1)  # Adjust the delay time as 
    
    async def autoclick_links(self, chat_id):
        await self.client.connect()

        if not await self.client.is_user_authorized():
            await self.client.send_code_request(self.phone_number)
            try:
                await self.client.sign_in(self.phone_number, input('Inserisci il codice: '))
            except telethon.errors.rpcerrorlist.SessionPasswordNeededError:
                password = input('Inserisci la password di verifica in due passaggi: ')
                await self.client.sign_in(password=password)

        @self.client.on(telethon.events.NewMessage(chats=chat_id))
        async def click_link(event):
                ...
                for url in urls:
                    if 't.me/' in url:
                        try:
                            ...
                            print(f"Entrato nel gruppo tramite il link: {url}")
                        except Exception as e:
                            print(f"Errore nell'entrare nel gruppo {url}: {e}")


        print(f"In ascolto per i link nella chat {chat_id}...")
...
async def main():
    ...
    print("3. Autojoin group from link")
    ...
    if choice == "1":
        ...
        chat_id = int(input("Inserisci l'ID della chat da monitorare per i link: "))
        await forwarder.autoclick_links(chat_id)
    else:
